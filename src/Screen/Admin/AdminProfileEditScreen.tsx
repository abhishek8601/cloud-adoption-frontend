...
export default function AdminProfileEditScreen() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      } catch (loadError) { setError(loadError instanceof Error ? loadError.message : 'Unable to load your profile.'); } finally { setIsLoading(false); }
    };
    void load();
  }, [user?.token]);
  ...
  return <View style={styles.screen}><StatusBar style="dark" /><SafeAreaView style={styles.safeArea}>
    <View style={styles.header}><Pressable disabled={isSaving} onPress={() => router.back()}><Text style={styles.back}>Back to Profile</Text></Pressable><Pressable disabled={isSaving} onPress={saveProfile} style={[styles.savePill, isSaving && styles.disabled]}>{isSaving ? <ActivityIndicator color="#FFFFFF" size="small" /> : <Text style={styles.savePillText}>Save</Text>}</Pressable></View>
    ...
    showsVerticalScrollIndicator={false}>
      {isLoading ? <View style={styles.loading}><ActivityIndicator color="#7C3AED" size="large" /><Text style={styles.loadingText}>Loading profile...</Text></View> : <><View style={styles.profileCard}><View style={styles.avatar}><Text style={styles.avatarText}>{initials}</Text></View><View><Text style={styles.name}>{name}</Text><Text style={styles.company}>{form.company_name || '-'}</Text><Text style={styles.role}>{role}</Text></View></View>
