...
export default function UserRegistrationScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.name || !form.email || !form.password || !form.passwordConfirmation || !form.phone || !form.company || !form.designation || !form.interest || !form.ticketReference || !form.city || !form.state) {
    ...
    try {
      ...
      if (!isSuccess) {
        ...
        throw new Error(data.message || firstValidationError || 'Registration failed. Please try again.');
      }

      // Registration successful — user must verify email, then wait for admin approval
      const successMessage =
        data.message ||
        'Please verify your email. After verification your account will be reviewed by the administrator.';
      if (Platform.OS === 'web') {
        (globalThis as any).alert(`Registration Successful\n\n${successMessage}`);
        router.replace('/');
      } else {
