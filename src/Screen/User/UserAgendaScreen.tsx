...
import { api, type AgendaItem } from '../../services/api';
...
const SESSION_COLORS = ['#7C3AED', '#00A878', '#F49A00', '#8B55ED', '#E51D47', '#0D9AAF'];
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        ...
        setFetchError('');
        if (items.length) {
          const first = toLocalDate(items[0].session_date);
          setSelectedDate(first);
          setCalendarMonth(new Date(first.getFullYear(), first.getMonth(), 1));
        }
      })
      .catch((e) => setFetchError(e instanceof Error ? e.message : 'Failed to load agenda.'))
    ...
    <View style={styles.screen}>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} style={styles.scroll}>
          {isFetching ? (
            <Text style={{ padding: 24, textAlign: 'center', color: '#718098' }}>Loading agenda...</Text>
          ) : fetchError ? (
            <Text style={{ padding: 24, textAlign: 'center', color: '#D92D3A' }}>{fetchError}</Text>
          ) : daySessions.length === 0 ? (
            <Text style={{ padding: 24, textAlign: 'center', color: '#718098' }}>No sessions scheduled for this date.</Text>
          ) : (
            daySessions.map((item, index) => (
              <SessionCard
                key={item.id}
                track={item.room || 'Session'}
                title={item.session_name}
                speaker={item.presenter}
                role={item.place || ''}
                initials={(item.presenter || '?')
                  .split(' ')
                  .map((part) => part[0])
                  .join('')
                  .toUpperCase()
                  .slice(0, 2)}
                time={new Intl.DateTimeFormat('en-US', { day: '2-digit', month: 'short' }).format(toLocalDate(item.session_date))}
                duration={/[a-zA-Z]/.test(String(item.duration)) ? String(item.duration) : `${item.duration} min`}
                room={[item.place, item.room].filter(Boolean).join(' · ')}
