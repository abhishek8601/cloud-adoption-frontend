...
import { SafeAreaView } from 'react-native-safe-area-context';
import { useAuth } from '../../context/AuthContext';
import { api, type AgendaItem, type ConferenceInfo } from '../../services/api';
...
export default function UserDashboardScreen() {
  ...
  useEffect(() => {
    ...
    if (user.conference_id) {
      api
        .conference(user.conference_id, token)
        .then((res) => setConference(res.data || null))
        .catch(() => { });
    }
    api
      .myAgenda(token)
    ...
    api
      .infographics(token)
      .then((res) => setTotalAttendees(res.data?.total_attendees ?? null))
      .catch(() => { });
  }, [user?.token, user?.conference_id]);

  const conferenceTitle = conference?.title || user?.conference_title || 'My Conference';
  const conferenceDate = conference?.conference_date
    ? new Intl.DateTimeFormat('en-US', { month: 'long', day: 'numeric', year: 'numeric' }).format(
      new Date(`${conference.conference_date}T00:00:00`),
    )
    : '—';
  const venueLine = [conference?.venue_name, conference?.venue_city, conference?.venue_state]
    .filter(Boolean)
    .join(', ') || '—';
  const addressLine = conference?.venue_address || '—';
  const sessionDays = new Set(agenda.map((a) => a.session_date)).size;
  const attendeeCount = totalAttendees !== null ? String(totalAttendees) : '—';
  const sessionCount = agenda.length ? String(agenda.length) : '—';
      ...
      <SafeAreaView style={styles.safeArea}>
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.eventCard}>
            <View style={styles.eventCopy}>
              <Text style={styles.eventEyebrow}>CLOUD ADOPTION SOLUTIONS</Text>
              <Text style={styles.eventTitle}>{conferenceTitle}</Text>
            </View>
            <View style={styles.logoBox}>
              <Image contentFit="contain" source={require('../../../assets/images/icon.png')} style={styles.logo} />
            </View>
            <View style={styles.divider} />
            <View style={styles.eventDetails}>
              <View style={styles.detailLine}><SymbolView name={{ ios: 'calendar', android: 'calendar_month', web: 'calendar_month' }} size={13} tintColor="#FFFFFF" /><Text style={styles.eventDetailText}>{conferenceDate}</Text></View>
              <View style={styles.detailLine}><SymbolView name={{ ios: 'mappin.and.ellipse', android: 'location_on', web: 'location_on' }} size={13} tintColor="#FFFFFF" /><Text style={styles.eventDetailText}>{venueLine}</Text></View>
              <View
                style={styles.detailLine}>
                <SymbolView name={{ ios: 'globe', android: 'language', web: 'language' }} size={13} tintColor="#FFFFFF" /><Text
                  style={styles.eventDetailText}>{addressLine}</Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.welcomeCard}>
            <View style={styles.avatar}><Text style={styles.avatarText}>{userInitials}</Text></View>
            <View style={styles.welcomeCopy}><Text style={styles.welcomeTitle}>Welcome back, {userName}</Text><Text style={styles.welcomeRole}>{[user?.company_name, user?.designation].filter(Boolean).join(' · ') || 'Attendee'}</Text></View>
            <View style={styles.verified}><SymbolView name={{ ios: 'checkmark', android: 'check', web: 'check' }} size={10} tintColor="#FFFFFF" /><Text style={styles.verifiedText}>Verified</Text></View>
          </View>
          ...
          <View style={styles.statsRow}>
            ...
            <Stat value={sessionCount} label="Sessions" symbol={{ ios: 'book', android: 'menu_book', web: 'menu_book' }} />
            <Stat value={sessionDays ? String(sessionDays) : '—'} label="Days" symbol={{ ios: 'calendar', android: 'calendar_month', web: 'calendar_month' }} />
          </View>
        </ScrollView>
