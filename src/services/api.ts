...
export const api = {
  ...
  /** Request a password-reset email. Used by the forgot-password form and the resend button. */
  forgotPassword: (email: string) => {
    const endpoint = process.env.EXPO_PUBLIC_FORGOT_PASSWORD_URL;
    if (!endpoint) {
    ...
    return apiRequest(endpoint, { method: "POST", body: { email } });
  },

  /**
   * Re-send the account verification email for an unverified user.
   * Requires EXPO_PUBLIC_RESEND_VERIFICATION_URL — the backend must expose a
   * public route that accepts { email } (the user has no token yet).
   */
  resendVerification: (email: string) => {
    const endpoint = process.env.EXPO_PUBLIC_RESEND_VERIFICATION_URL;
    if (!endpoint) {
      return Promise.reject(
        new Error(
          "Resending verification emails is not configured yet. Please contact support.",
        ),
      );
    }
    return apiRequest(endpoint, { method: "POST", body: { email } });
  ...
  changePassword: (
    ...
    apiRequest(
      process.env.EXPO_PUBLIC_CHANGE_PASSWORD_URL || "/change-password",
      {
        method: "PUT",
        token,
        body: payload,
...
export const adminApi = {
  conferenceAttendees: (token: string) => {
    ...
    if (!endpoint) {
      return Promise.reject(
        new Error("Conference attendees endpoint is not configured."),
      );
    }
